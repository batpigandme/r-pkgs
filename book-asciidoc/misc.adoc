...
* `+inst/+`: for arbitrary additional files that you want include in your package. This includes a few special files, like the `+CITATION+`, described below in <<sec-misc-inst>>. Other examples of files that might appear below `+inst/+` include R Markdown templates (see `+usethis::use_rmarkdown_template()+`) or https://docs.posit.co/ide/user/ide/guide/productivity/add-ins.html[RStudio add-ins].
